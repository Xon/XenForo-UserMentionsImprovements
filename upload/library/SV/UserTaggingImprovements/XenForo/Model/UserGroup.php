...
class SV_UserTaggingImprovements_XenForo_Model_UserGroup extends XFCP_SV_UserTaggingImprovements_XenForo_Model_UserGroup
{
    public function updateUserGroupAndPermissions($userGroupId, array $userGroupInfo, array $permissions)
    {
        if (!empty(SV_UserTaggingImprovements_Globals::$UserGroupAdminController))
        {
            $input = SV_UserTaggingImprovements_Globals::$UserGroupAdminController->getInput();
